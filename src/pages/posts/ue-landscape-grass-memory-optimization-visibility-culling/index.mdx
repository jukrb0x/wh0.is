...
但 $V_{cam} \cdot V_{sub} = 0 $ 时，一整排的草地也会被载入，这显然不是我们想要的最优解。同时，$V*{cam} \cdot V*{sub} \to 0-$ 的地块 $V_{sub}$ 草同样会消失：

![统](./grass-orthogonal.png)
...
如果以为这就是优化方案的结束，那你就错了，由源码分析可得我们还有一个变量 `GMaxInstancesPerComponent` 用于控制每个 **subsection** 上的最多 instance 数量（并不是语义上的 landscape component... ），引擎默认设置为 65536 ($2^{16}$)，下限为 1024 ($2^{10}$)，没有上限。
