...
Nextra uses Next Image to wrap all `<img>` from MDX files, all links and images will be optimized auomatically by Nextra,
...
This is much convenient for post asset management, I can handle all images along with the post under a signle directory:
...
But there are some issues for the image handling by Nextra, I cannot easily custom an MDX image component as I want since it will break the static-import workflow. Ultimately, Nextra uses a MDX plugin to parse all images into statically imported images wrapped by `next/image`. Also, The relative URL is not handled for the Open Graph Image (og:image). I am still working on unifying this.
